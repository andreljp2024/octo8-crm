...
const MOCK_MESSAGES = [
  { id: 'm-1', sender: 'CUSTOMER', text: 'Bom dia, minha internet parou de funcionar do nada.', time: '10:42' },
  { id: 'm-2', sender: 'BOT', text: 'Olá! Sou o assistente virtual. Para agilizar, pode me confirmar seu CPF?', time: '10:42' },
  { id: 'm-3', sender: 'CUSTOMER', text: '111.222.333-44', time: '10:43' },
  { id: 'm-4', sender: 'BOT', text: 'Obrigado. Identifiquei uma instabilidade na sua região. Vou transferir para um especialista.', time: '10:43' },
  { id: 'm-5', sender: 'SYSTEM', text: 'Atendimento transferido da fila: Triage_Bot para: Suporte_N1', time: '10:45' },
];
    ...
    <div className="h-[calc(100vh-8rem)] flex bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden">
        ...
        <div className="h-16 px-6 border-b border-slate-200 bg-white flex items-center justify-between shrink-0">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-full bg-slate-200 flex items-center justify-center text-slate-600 font-bold">
              {MOCK_CONVERSATIONS.find(c => c.id === activeConv)?.name.charAt(0)}
            </div>
            <div>
              <h2 className="font-bold text-slate-900">{MOCK_CONVERSATIONS.find(c => c.id === activeConv)?.name}</h2>
              <p className="text-xs text-slate-500 flex items-center gap-1">
                Via {MOCK_CONVERSATIONS.find(c => c.id === activeConv)?.channel}
              </p>
            </div>
          </div>
      ...
      <div className="w-80 flex-shrink-0 bg-white flex flex-col">
        ...
        <div className="p-6 border-b border-slate-200 text-center">
          ...
          <h3 className="font-bold text-slate-900">{MOCK_CONVERSATIONS.find(c => c.id === activeConv)?.name}</h3>
